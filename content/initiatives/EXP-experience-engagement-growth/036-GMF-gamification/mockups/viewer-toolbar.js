(function() {
  const screens = [
    { id: 1, short: 'Dashboard Widget', file: '01-dashboard-widget.html' },
    { id: 2, short: 'All Clear', file: '02-all-clear-celebration.html' },
    { id: 3, short: 'Challenges', file: '03-challenges-page.html' },
    { id: 4, short: 'Badges', file: '04-badges-profile.html' },
    { id: 5, short: 'Practice Flames', file: '05-practice-flames.html' },
  ];
  const currentFile = window.location.pathname.split('/').pop() || '';
  const currentIdx = screens.findIndex(s => currentFile.includes(s.file.replace('.html', '')));
  if (currentIdx === -1) return;
  if (window.location.hash.includes('figmaselector')) { return; }
      ...
      <div class="max-w-7xl mx-auto flex items-center justify-between">
        <div class="flex items-center gap-4">
          <div class="flex items-center gap-2">
            ...
            <h1 class="text-base font-semibold text-white">Gamification (036-GMF)</h1>
          </div>
          <span class="text-gray-600">|</span>
          <span class="text-gray-400 text-sm">${String(screens[currentIdx].id).padStart(2, '0')} — ${screens[currentIdx].short}</span>
        </div>
        <span class="text-xs text-gray-500">${currentIdx + 1} / ${screens.length}</span>
      </div>
    </div>
